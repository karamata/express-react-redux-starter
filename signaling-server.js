module.exports = exports = function(app, socketCallback) {
	...
	function onConnection(socket) {
		...
		socket.on('extra-data-updated', function(extra) {
			try {
				...
				for ( var user in listOfUsers[socket.userid].connectedWith) {
					listOfUsers[user].socket.emit('extra-data-updated',
							socket.userid, extra);
				}
			} catch (e) {
		...
		socket.on('get-remote-user-extra-data',
				function(remoteUserId, callback) {
					callback = callback || function() {
					};
					if (!remoteUserId || !listOfUsers[remoteUserId]) {
						callback('remoteUserId (' + remoteUserId
								+ ') does NOT exist.');
						return;
					}
...
function pushLogs() {
	...
	try {
		...
		require('fs').writeFileSync(logsFile, JSON.stringify(logs, null, '\t'));
	} catch (e) {
