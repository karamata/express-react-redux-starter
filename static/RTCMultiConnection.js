...
window.RTCMultiConnection = function(roomid, forceOptions) {
    ...
    function setHarkEvents(connection, streamEvent) {
        ...
        if(streamEvent.stream.isScreen === true) {
        	return;
        }
    ...
    (function(connection) {
        ...
        connection.getVideoStream = function() {
        	if(!!connection.attachStreams && connection.attachStreams.length > 0) {
        		for(var i = 0; i < connection.attachStreams.length; i++) {
        			if(connection.attachStreams[i].isVideo && !connection.attachStreams[i].isScreen) {
        				return connection.attachStreams[i];
        			}
        		}
        	}
        	return null;
        ...
        connection.getScreenStream = function() {
        	if(!!connection.attachStreams && connection.attachStreams.length > 0) {
        		for(var i = 0; i < connection.attachStreams.length; i++) {
        			if(connection.attachStreams[i].isVideo && connection.attachStreams[i].isScreen) {
        				return connection.attachStreams[i];
        ...
        connection.getVideoEventStream = function() {
        	var videoStream = connection.getVideoStream();
        	if(!videoStream) {
        		return null;
        	}
        	return connection.streamEvents[videoStream.id];
        };

        connection.getScreenEventStream = function() {
        	var screenStream = connection.getScreenStream();
        	if(!screenStream) {
        		return null;
        	}
        	return connection.streamEvents[screenStream.id];
        };

        connection.getPeerVideoStream = function(userid) {
        	var peer = connection.peers[userid];
        	if(!peer) {
        		return null;
        	}

        	for(var i = 0; i < peer.streams.length; i++) {
        		var stream = peer.streams[i];
        		if(stream.isVideo && !stream.isScreen) {
        			return peer.streams[i];
        		}
        	}
        	return null;
        }

        connection.getPeerScreenStream = function(userid) {
        	var peer = connection.peers[userid];
        	if(!peer) {
        		return null;
        	}
        ...
        connection.getPeerVideoEventStream = function(userid) {
        	var videoStream = connection.getPeerVideoStream(userid);
        	if(!videoStream) {
        		return null;
        	}
        	return connection.streamEvents[videoStream.id];
        ...
        connection.getPeerScreenEventStream = function(userid) {
        	var screenStream = connection.getPeerScreenStream(userid);
        	if(!screenStream) {
        		return null;
        	}
        	return connection.streamEvents[screenStream.id];
